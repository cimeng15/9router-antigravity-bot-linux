...
echo ""
echo "[INFO] Menginstall Camoufox di venv..."
"$VENV_DIR/bin/pip" install --upgrade pip 2>/dev/null || true
"$VENV_DIR/bin/pip" install "camoufox[geoip]"
"$VENV_DIR/bin/python" -m camoufox fetch

echo ""
echo "[OK] Camoufox berhasil diinstall! (browser anti-detect, Chrome tidak diperlukan)"
